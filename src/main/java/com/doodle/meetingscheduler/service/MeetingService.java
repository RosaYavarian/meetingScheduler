...
public class MeetingService {
    ...
    public MeetingService(CalendarRepository calendarRepository, TimeSlotRepository timeSlotRepository, MeetingRepository meetingRepository) {
    ...
    @Transactional
    public MeetingResponse createMeeting(UUID organizerId, UUID slotId, CreateMeetingRequest request) {
        Set<UUID> participantIds = new HashSet<>(request.participantIds());
        participantIds.remove(organizerId);
        Set<UUID> allUserIds = new HashSet<>(participantIds);
        allUserIds.add(organizerId);
        List<Calendar> calendars = calendarRepository.findAllByUserIdsForUpdate(allUserIds);
        validateAllUsersExist(allUserIds, calendars);
        TimeSlot organizerSlot = timeSlotRepository.findByIdAndCalendarUserId(slotId, organizerId)
        ...
        List<TimeSlot> availableSlots = timeSlotRepository.findAllByCalendarIdInAndStartTimeAndEndTimeAndStatus(calendarIds, organizerSlot.getStartTime(), organizerSlot.getEndTime(), SlotStatus.FREE);
        if (availableSlots.size() != allUserIds.size()) {
            throw new ParticipantUnavailableException();
        }
        User organizer = calendars.stream().map(Calendar::getUser)
                .filter(user -> user.getId().equals(organizerId))
                .findFirst()
                .orElseThrow(() -> new UserNotFoundException(organizerId));
        Set<User> participants = calendars.stream().map(Calendar::getUser)
                .filter(user -> participantIds.contains(user.getId()))
                .collect(Collectors.toSet());
        Meeting meeting = new Meeting(request.title(), request.description(), organizerSlot.getStartTime(), organizerSlot.getEndTime(), organizer, participants);
        Meeting savedMeeting = meetingRepository.saveAndFlush(meeting);
        availableSlots.forEach(slot -> slot.book(savedMeeting));
    ...
    private void validateAllUsersExist(Collection<UUID> requestedUserIds, List<Calendar> calendars) {
        if (calendars.size() != requestedUserIds.size()) {
            throw new UserNotFoundException(findMissingUserId(requestedUserIds, calendars));
        }
    }

    private UUID findMissingUserId(Collection<UUID> requestedUserIds, List<Calendar> calendars) {
        Set<UUID> foundUserIds = calendars.stream().map(Calendar::getUser).map(User::getId).collect(Collectors.toSet());
        return requestedUserIds.stream().filter(id -> !foundUserIds.contains(id)).findFirst().orElseThrow();
    }

    private MeetingResponse toResponse(Meeting meeting) {
        Set<UUID> participantIds = meeting.getParticipants().stream().map(User::getId).collect(Collectors.toSet());
        return new MeetingResponse(meeting.getId(), meeting.getTitle(), meeting.getDescription(), meeting.getStartTime(), meeting.getEndTime(), meeting.getOrganizer().getId(), participantIds);
    }
}