...
(
    ...
    CONSTRAINT chk_meetings_time_range CHECK (start_time < end_time),
...
(
    ...
    CONSTRAINT chk_time_slots_status CHECK (status IN ('FREE', 'BUSY')),

    CONSTRAINT chk_meeting_slot_is_busy CHECK (meeting_id IS NULL OR status = 'BUSY'),

    CONSTRAINT fk_time_slots_calendar FOREIGN KEY (calendar_id) REFERENCES calendars (id) ON DELETE CASCADE,

    CONSTRAINT fk_time_slots_meeting FOREIGN KEY (meeting_id) REFERENCES meetings (id) ON DELETE RESTRICT,

    CONSTRAINT  no_overlapping_time_slots EXCLUDE USING gist (
            calendar_id WITH =,
...
CREATE INDEX idx_time_slots_meeting ON time_slots (meeting_id) WHERE meeting_id IS NOT NULL;
